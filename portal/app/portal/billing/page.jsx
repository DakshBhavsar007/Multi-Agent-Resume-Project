  ...
  const handleUpgrade = async (planId) => {
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        key: orderData.razorpay_key_id || "rzp_test_mock", // mock fallback
        ...
        handler: async function(response) {
          try {
            ...
            // Mock state update
            setAuth({ ...usePortalAuthStore.getState(), tier: planId });
      ...
      // Mock success if razorpay undefined
      if (!window.Razorpay) {
         setTimeout(()=> {
         ...
         <div className="flex flex-col gap-2 pl-2">
            ...
            <div className="flex items-center gap-4">
              <h2 className="text-3xl font-black text-charcoal flex items-center gap-3">
                 <span className="text-2xl">📦</span> {plans.find(p=>p.id===activePlan)?.name || "Free"} Plan
              </h2>
           ...
           return (
             <div key={p.id} className={`flex flex-col border-2 rounded-3xl p-6 bg-white transition-all relative overflow-hidden ${isActive ? "border-accent shadow-xl shadow-amber-500/10 scale-105 z-10" : "border-gray-100"}`}>
               ...
               <div className="flex justify-between items-center mb-4">
                 ...
                 {isActive && <span className="bg-amber-100 text-amber-800 text-[10px] font-black uppercase px-2 py-1 rounded-full">Current</span>}
               </div>
