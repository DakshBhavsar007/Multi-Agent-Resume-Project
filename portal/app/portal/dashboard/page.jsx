...
import { Activity, FileText, Zap, Key, Crown } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const customPieLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, value, index }) => null; // Using tooltip instead of external labels to keep pie clean
    ...
    <div className="w-full max-w-7xl mx-auto pb-12">
      ...
      {highestUsage > 80 && (
         <div className="mb-8 w-full bg-amber-50 border border-accent/30 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
            ...
            <a href="/portal/billing" className="px-5 py-2 bg-accent text-white text-sm font-bold rounded-lg hover:bg-accent-dark transition-colors whitespace-nowrap">Upgrade Plan</a>
         </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
         ...
         <div className="bg-gradient-to-br from-amber-500 to-amber-600 border border-amber-600 rounded-2xl p-5 flex flex-col shadow-lg shadow-amber-500/20 text-white">
            ...
            {(!tier || tier === "free" || tier === "starter") && (
              <a href="/portal/billing" className="text-[11px] font-black uppercase text-white/90 hover:text-white mt-auto underline decoration-white/40 underline-offset-2">Upgrade &rarr;</a>
            )}
         </div>
      </div>
