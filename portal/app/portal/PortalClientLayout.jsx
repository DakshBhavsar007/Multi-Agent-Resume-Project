...
export default function PortalClientLayout({ children }) {
  const { jwt, developer, tier, company_name, initFromStorage, clearAuth } = usePortalAuthStore();
      ...
      <aside className={`${mobileMenu ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:relative z-40 w-64 h-full bg-white border-r border-[#e6dfcd] flex flex-col transition-transform duration-300 ease-in-out`}>
        ...
        <div className="p-6 pb-2">
          <div className="flex items-start flex-col mb-8 gap-0.5 mt-4 md:mt-0">
             <span className="text-2xl font-black text-accent tracking-tight">Vishleshan</span>
          ...
          <div className="flex flex-col gap-1 mb-4 border-t border-[#e6dfcd] pt-4">
            {bottomItems.map((item) => {
              ...
              return (
                <button
                  key={item.href}
                  onClick={() => { if(item.href.startsWith("/")) router.push(item.href); setMobileMenu(false); }}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold transition-all ${
             ...
             <button
                onClick={clearAuth}
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold text-red-500 hover:bg-red-50 transition-all mt-2"
              >
