...
import { LayoutTemplate, Plus, Lock, X, Component, Shield, CheckCircle } from "lucide-react";
    ...
    return (
      <div className="w-full max-w-5xl mx-auto flex items-center justify-center min-h-[60vh] relative">
         <div className="bg-white border-2 border-gray-100 rounded-3xl p-10 max-w-lg w-full flex flex-col items-center text-center shadow-2xl relative z-10">
            ...
            <a href="/portal/billing" className="w-full py-4 bg-accent text-white font-bold rounded-2xl hover:bg-accent-dark transition-all">Upgrade to Business &rarr;</a>
         </div>
      </div>
    );
  }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal/40 backdrop-blur-sm">
            <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} exit={{opacity:0, scale:0.95}} className="bg-[#1E1E1E] rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden relative border border-gray-800">
              ...
              <div className="p-6 relative">
                 <button onClick={() => {navigator.clipboard.writeText(codeSnippets[activeTab]); toast.success("Code copied!")}} className="absolute top-6 right-6 px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-xs font-bold rounded shadow transition-colors">Copy Code</button>
