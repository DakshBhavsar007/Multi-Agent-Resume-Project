...
const COLORS = { Parse: "#C8871A", Match: "#3B82F6", Chat: "#22C55E" };
...
export default function UsagePage() {
  ...
  // Mocking endpoint data since it might not be in api client yet
  ...
  const downloadCSV = () => {
     // rudimentary CSV generator based on timeline
     if (!timeline) return;
     let csv = "Date,Parse,Match,Chat\n";
     timeline.forEach(row => {
        csv += `${row.date},${row.parse || 0},${row.match || 0},${row.chat || 0}\n`;
     });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
         {[{label: "Total Calls", val: totalCalls}, {label: "Parses", val: parseUsage}, {label: "Matches", val: matchUsage}, {label: "Chats", val: chatUsage}, {label: "Avg Latency", val: avgLatency}].map((s, i) => (
         ...
         <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col">
            ...
            <div className="flex-1 overflow-x-auto">
               <table className="w-full text-left text-sm">
                 ...
                 <tbody>
                   {/* Generating mock backwards 6 months history based on total calls */}
                   {Array.from({length: 6}).map((_, i) => {
                     ...
                     const p = Math.floor(mockCalls * 0.6);
                     const mt = mockCalls - p;
                     return (
                       <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
