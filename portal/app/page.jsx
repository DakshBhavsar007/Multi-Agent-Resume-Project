    ...
    <div className="min-h-screen font-sans text-charcoal bg-bg">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-white shadow-md py-3" : "bg-white/90 backdrop-blur-md py-4"}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-accent">Vishleshan</span>
          ...
          <div className="hidden md:flex items-center gap-4">
            <a href="/login" className="px-5 py-2 rounded-lg text-accent border border-accent font-semibold hover:bg-accent-light transition-colors">Sign In</a>
            <a href="/register" className="px-5 py-2 rounded-lg bg-accent text-white font-semibold hover:bg-accent-dark transition-colors shadow-sm">Get API Key</a>
          </div>
      ...
      {mobileMenu && (
        <div className="fixed inset-0 top-[60px] bg-white z-40 p-6 flex flex-col gap-6 md:hidden">
            ...
            <a href="#status" className="text-lg font-semibold text-gray-700" onClick={() => setMobileMenu(false)}>Status</a>
            <div className="border-t pt-6 flex flex-col gap-4">
               <a href="/login" className="w-full text-center px-5 py-3 rounded-lg text-accent border border-accent font-semibold">Sign In</a>
               <a href="/register" className="w-full text-center px-5 py-3 rounded-lg bg-accent text-white font-semibold">Get API Key</a>
            </div>
        </div>
      )}
      ...
      <header className="pt-32 pb-20 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        <div className="md:col-span-5 flex flex-col items-start gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto mt-2">
             <a href="/register" className="flex justify-center items-center px-6 py-3.5 rounded-xl bg-accent text-white font-bold hover:bg-accent-dark transition-all hover:-translate-y-1 hover:shadow-lg hover:shadow-accent/30 gap-2">
               Get Free API Key <span className="text-xl leading-none">→</span>
             </a>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center max-w-5xl mx-auto">
             {plans.map((plan, i) => (
                <div key={plan.id} className={`bg-white rounded-3xl p-8 shadow-xl ${plan.id === 'starter' ? 'border-2 border-accent md:-translate-y-4 shadow-amber-500/10' : 'border border-gray-100'}`}>
                   ...
                   <a href="/register" className={`w-full block text-center py-3.5 rounded-xl font-bold transition-all ${plan.id === 'starter' ? 'bg-accent text-white hover:bg-accent-dark shadow-md shadow-accent/20' : 'bg-gray-100 text-charcoal hover:bg-gray-200'}`}>
