...
import { Check, X, GripVertical, Plus, ArrowRight, ArrowLeft, Loader2, Save } from 'lucide-react';
...
export default function NewSessionWizard() {
  ...
  const handleInfer = async () => {
    ...
    try {
      ...
      const inferred = await sessionsAPI.inferSkills(session.id, {
        job_description: formData.job_description
      });
      
      setInferredData({ ...inferred, session_id: session.id });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/sessions/${currentSessionId}`);
    } catch (err) {
        ...
        return (
          <div className="bg-white rounded-2xl p-8 shadow-[0_4px_24px_rgba(0,0,0,0.06)] w-full max-w-[640px] mx-auto">
            ...
            <div className="space-y-5">
              ...
              >
                {inferring ? <Loader2 className="animate-spin" size={18} /> : <span>✨</span>}
              ...
              {inferredData && (
                <div className="mt-4 border-[1.5px] border-green-200 bg-green-50 rounded-xl p-4">
                  <div className="text-green-800 font-medium text-sm mb-3">
                    ✅ AI found requirements for: {formData.job_title}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {formData.preferred_locations.map((l, i) => (
                      <span key={i} className="text-[11px] bg-blue-100 text-blue-800 px-2 py-1 rounded font-semibold border border-blue-200">📍 {l}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-gray-100">
                ...
                <div className="mt-4 flex justify-between items-center text-sm font-medium">
                  ...
                  {Math.abs(weightsSum - 1.0) > 0.01 ? (
                    <span className="text-red-500 mb-0">⚠️ Must equal exactly 1.0</span>
                  ) : (
