...
1. For each experience entry, rewrite the `responsibilities` array:
   ...
   - Make EVERY bullet noticeably stronger — vague bullets should become sharp and impactful.

2. For each project entry, rewrite bullet points in the `description` field:
   ...
   - Make the impact and contribution crystal clear.
...
6. SCORING RULES (CRITICAL — MUST FOLLOW):
   - The `ats_score_original` MUST equal {live_ats_score} (the live score provided above).
   - The `ats_score_enhanced` MUST be STRICTLY HIGHER than `ats_score_original`.
   - The minimum boost is +10 points. Aim for +12 to +20 points for meaningful enhancement.
   - The enhanced score reflects applying ALL your suggested improvements: better bullets, missing keywords added, summary rewrite, and improved quantification.
   - Maximum allowed score is 100.
   - NEVER return ats_score_enhanced <= ats_score_original. This is a hard rule.

OUTPUT FORMAT — return ONLY this JSON, filled in with real data:
{{
  "ats_score_original": {live_ats_score},
  "ats_score_enhanced": <integer strictly greater than {live_ats_score}, minimum {live_ats_score_plus_10}>,
...
class ResumeEnhancerAgent:
    ...
    def enhance(self, resume_data: dict, job_description: str = "", live_ats_score: int = None) -> dict:
        ...
        Args:
            ...
            live_ats_score: The live ATS score from the frontend sidebar (overrides LLM's original score).
        ...
        try:
            ...
            # Use live score if provided, otherwise let LLM estimate
            base_score = int(live_ats_score) if live_ats_score is not None else 70
            min_enhanced = min(100, base_score + 10)

            prompt = _ENHANCE_PROMPT.format(
                resume_json=resume_json,
                job_description=jd_text[:3000],  # cap JD at 3k chars
                live_ats_score=base_score,
                live_ats_score_plus_10=min_enhanced,
            )
            ...
            response = self.llm.chat.completions.create(
                ...
                temperature=0.25,             # slightly higher for more creative bullet rewrites
            ...
            # Ensure the summary rewrite ends with a period if it doesn't already
            summary_enhanced = result.get("professional_summary_enhanced", "")
            if summary_enhanced:
                summary_enhanced = summary_enhanced.strip()
                if summary_enhanced and not summary_enhanced.endswith("."):
                    summary_enhanced += "."
                result["professional_summary_enhanced"] = summary_enhanced

            # Always override original score with live score from frontend (source of truth)
            if live_ats_score is not None:
            ...
            # HARD GUARANTEE: enhanced must always be at least 10 points above original
            floor = result["ats_score_original"] + 10
            if result["ats_score_enhanced"] < floor:
                result["ats_score_enhanced"] = min(100, floor)
