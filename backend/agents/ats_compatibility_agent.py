...
class AtsCompatibilityAgent:
    ...
    def _check_spelling_and_grammar(self, resume_text: str, parsed_data: dict, target_job_description: str = None) -> tuple:
        score = 100
        spelling_issues = []
        grammar_issues = []
        ...
        try:
            ...
            # If target job description is provided, whitelist words from JD that match TECH_DICT
            if target_job_description:
                for word in re.findall(r'\b[a-zA-Z]{3,22}\b', target_job_description):
                    word_lower = word.lower()
                    if word_lower in TECH_DICT:
    ...
    def _check_keywords(self, resume_text: str, parsed_data: dict, target_jd: str = None) -> tuple:
        ...
        # Clean missing and matched lists: remove any missing keyword that is already matched
        clean_missing = []
        for m in missing:
            m_clean = m.strip().lower()
            if m_clean in resume_skills:
                if m not in matched:
                    matched.append(m)
                continue
            # Check if present as a distinct word in the resume text
            pattern = re.compile(rf'\b{re.escape(m_clean)}\b', re.IGNORECASE)
        ...
        # Unique case-insensitive matched list
        unique_matched = []
        seen_matched = set()
        for m in matched:
            m_lower = m.lower().strip()
            if m_lower not in seen_matched:
                seen_matched.add(m_lower)
    ...
    def analyze(self, resume_text: str, parsed_data: dict, target_job_description: str = None) -> dict:
        ...
        spelling_score, spelling_issues = self._check_spelling_and_grammar(resume_text, parsed_data, target_job_description)
