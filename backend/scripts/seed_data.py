...
async def seed_skill_taxonomy(db):
    """Called on startup. Inserts skills if table empty."""
    count_res = await db.execute(select(func.count(SkillTaxonomy.id)))
    ...
    await db.commit()
    print(f"[OK] Seeded {len(SKILL_SEED_DATA)} skills")
